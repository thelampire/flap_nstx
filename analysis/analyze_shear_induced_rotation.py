        ...
        if plot_shear_profile:
            if (shot_to_plot is None or
                ...
                if save_data_into_txt:
                    ...
                    rad_coord=shear_data['coord']['r']['values']
                    if save_data_filename is None:
                        file1=open(wd+'/shear_induced_rotation_profile.txt', 'w+')
                    else:
                        file1=open(save_data_filename, 'w+')
                    file1.write('#Radial coordinate (m)\n')
                    for i in range(1, len(rad_coord)):
                        file1.write(str(rad_coord[i])+'\t')

                    file1.write('\n#vpol profile [km/s]\n')
                    for i in range(1, len(vpol_rad_smooth/1e3)):
                        file1.write(str((vpol_rad_smooth/1e3)[i])+'\t')

                    file1.write('\n#vpol_prime profile  [1e3/s]\n')
                    for i in range(1, len(vpol_prime_smooth/1e3)):
                        file1.write(str((vpol_prime_smooth/1e3)[i])+'\t')
