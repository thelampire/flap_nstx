...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 15 00:44:36 2022
...
from flap_nstx.analysis import get_elms_with_thomson_profile
...
"""
Same as plot_elm_trans_vs_profiles but with rotation
"""

def plot_elm_rotation_vs_gradient(elm_duration=100e-6,
                                  recalc=False,
                                  plot=False,
                                  pdf=False,
    ...
    if spline_thomson:
        result_filename=wd+'/processed_data/rotation_vs_profile_spline'
    else:
        result_filename=wd+'/processed_data/rotation_vs_profile'
    ...
    if not os.path.exists(scaling_db_file) or recalc:
        ...
        for elm in elms_with_thomson:
            ...
            if not spline_thomson:
                gradient_error['Pressure'].append(get_nstx_thomson_gradient(exp_id=elm['shot'],
                                                                            pressure=True,
                                                                            r_pos=r_at_separatrix,
                                                                            force_mdsplus=force_mdsplus).slice_data(slicing=time_slicing).error)
                gradient_error['Density'].append(get_nstx_thomson_gradient(exp_id=elm['shot'],
                                                                           density=True,
                                                                           r_pos=r_at_separatrix,
                                                                           force_mdsplus=force_mdsplus).slice_data(slicing=time_slicing).error)
                gradient_error['Temperature'].append(get_nstx_thomson_gradient(exp_id=elm['shot'],
                                                                               temperature=True,
                                                                               r_pos=r_at_separatrix,
                                                                               force_mdsplus=force_mdsplus
                                                                               ).slice_data(slicing=time_slicing).error)
            ...
            if status != 'NO':
                ...
                filename=flap_nstx.tools.filename(exp_id=elm['shot'],
                ...
                rotation_results=pickle.load(open(filename+'.pickle', 'rb'))

                for key in gpi_results_avg:
                    ind_nan=np.isnan(rotation_results[key])
                    rotation_results[key][ind_nan]=0.
                ...
                for key in gpi_results_avg:
                    data_roi = rotation_results[key][elm_time_ind:elm_time_ind+nwin]
                    gpi_results_avg[key].append(np.mean(data_roi,axis=0))
                    
                    ind_max = np.argmax(np.abs(data_roi),axis=0)
                    gpi_results_max[key].append(data_roi[ind_max])
    ...
    if plot_preliminary:
        ...
        for var_ind in range(len(y_variables)):
            if pdf:
                pdf_pages=PdfPages(result_filename.replace('processed_data', 'plots')+'_'+title_addon[var_ind].replace('(','').replace(')','').replace(' ','_')+'.pdf')
    ...
    if not plot:
        plt.close('all')
    if return_results:
       return gradient, gradient_error, gpi_results_avg, gpi_results_max
        
        
def plot_elm_rotation_vs_max_gradient(elm_duration=100e-6,
    ...
    wd=flap.config.get_all_section('Module NSTX_GPI')['Working directory']
    if spline_thomson:
        result_filename=wd+'/processed_data/rotation_vs_profiles_spline'
    else:
        result_filename=wd+'/processed_data/rotation_vs_profiles'
    ...
    if not os.path.exists(scaling_db_file) or recalc:
        ...
        for elm in elms_with_thomson:
            ...
            if status != 'NO':
                ...
                elm_time_ind=int(np.argmin(np.abs(time-elm_time)))
                for key in gpi_results_avg:
                    gpi_results_avg[key].append(np.mean(rotation_results[key][elm_time_ind:elm_time_ind+nwin],axis=0))
                    gpi_results_max[key].append(np.max(np.abs(rotation_results[key][elm_time_ind:elm_time_ind+nwin]),axis=0))
                if thomson_before:
                    time_slicing={'Time':elm['thomson_time']}
    ...
    if return_results:
        return gradient, gradient_thom_cor, gpi_results_avg, gpi_results_max 
        
def plot_elm_rotation_vs_gradient_before_vs_after(elm_window=600e-6,
                                                    elm_duration=100e-6,
            ...
            for elm in elms_with_thomson:
                ...
                if (status != 'NO' and 
                    ...
                    filename=flap_nstx.tools.filename(exp_id=shot,
                                                  working_directory=wd+'/processed_data',
                                                  time_range=[elm_time-elm_window,elm_time+elm_window],
                                                  purpose='ccf ang velocity',
                                                  comment='pfit_o'+str(subtraction_order)+\
                                                          '_fst_0.0')
                    rotation_results=pickle.load(open(filename+'.pickle', 'rb'))
                    rotation_results['Velocity ccf'][np.where(rotation_results['Correlation max'] < correlation_threshold),:]=[np.nan,np.nan]
                    for key in gpi_results_avg:
                        ind_nan=np.isnan(rotation_results[key])
                        rotation_results[key][ind_nan]=0.
                    time=rotation_results['Time']
                    ...
                    for key in gpi_results_avg:
                        if averaging == 'before_after':
                            ...
                            if ind_before_after == 0:
                                gpi_results_avg[key].append(np.mean(rotation_results[key][elm_time_ind-nwin:elm_time_ind],axis=0))
                            elif ind_before_after == 1:
                                gpi_results_avg[key].append(np.mean(rotation_results[key][elm_time_ind+n_elm:elm_time_ind+nwin],axis=0))
                        elif averaging == 'full':
                            gpi_results_avg[key].append(np.mean(rotation_results[key][elm_time_ind+n_elm:elm_time_ind+nwin],axis=0))                            
                            
                        elif averaging == 'elm':
                            gpi_results_avg[key].append(rotation_results[key][elm_time_ind])
                            
                        if len(rotation_results[key][elm_time_ind-nwin:elm_time_ind+nwin].shape) == 2:
                            
                            max_ind_0=np.argmax(np.abs(rotation_results[key][elm_time_ind-nwin:elm_time_ind+nwin,0]),axis=0)
                            max_ind_1=np.argmax(np.abs(rotation_results[key][elm_time_ind-nwin:elm_time_ind+nwin,1]),axis=0)
                            gpi_results_max[key].append([rotation_results[key][elm_time_ind-nwin:elm_time_ind+nwin,0][max_ind_0],
                                                         rotation_results[key][elm_time_ind-nwin:elm_time_ind+nwin,1][max_ind_1]])
                        else:
                            max_ind=np.argmax(np.abs(rotation_results[key][elm_time_ind-nwin:elm_time_ind+nwin]),axis=0)
                            gpi_results_max[key].append(rotation_results[key][elm_time_ind-nwin:elm_time_ind+nwin][max_ind])
...
#                                                       d_err*np.abs((a-b)/2*(c-x)/(2*d**2)*(1-np.tanh((c-x)/(2*d))))
    ...
    if return_results:
        ...
        return_dict={'gradient_before':gradient_before,
                     'gradient_before_error':gradient_before_error,
                     'gpi_results_avg_before':gpi_results_avg_before,
                     'gpi_results_max_before':gpi_results_max_before,
                     
                     'gradient_after':gradient_after,
                     'gradient_after_error':gradient_after_error,
                     'gpi_results_avg_after':gpi_results_avg_after,
                     'gpi_results_max_after':gpi_results_max_after,}
        return return_dict
    
    
    
def plot_elm_rotation_vs_ahmed_fitting(averaging='before',
                                       parameter='grad_glob',
                                       subtraction_order=4,
                                       test=False,
                                       recalc=True,
    ...
    #     raise ValueError('Averaging should be one of the following: before_after, full, elm')
    # if parameter not in ['grad_glob','ped_height','value_at_max_grad', 'max_grad', 'ped_width', 'max_grad_position']:
    ...
    result_filename=wd+'/processed_data/'+'rotation_vs_profiles_ahmed_db'
    ...
    if not os.path.exists(scaling_db_file) or recalc:
        ...
        for elm_ind in elm_index:
            ...
            if status != 'NO':
                ...
                for key in ['Velocity ccf FLAP', 
                            'Velocity ccf skim']:
                    orig=copy.deepcopy(rotation_results[key])
                    rotation_results[key][:,0]=coeff_r_new/det*(coeff_z[1]*orig[:,0]-coeff_r[1]*orig[:,1])
                    rotation_results[key][:,1]=coeff_z_new/det*(-coeff_z[0]*orig[:,0]+coeff_r[0]*orig[:,1])
                
                rotation_results['Velocity ccf FLAP'][np.where(rotation_results['Correlation max'] < correlation_threshold),:]=[np.nan,np.nan]
                for key in gpi_results_avg:
                    ind_nan=np.isnan(rotation_results[key])
                    rotation_results[key][ind_nan]=0.
                time=rotation_results['Time']
                ...
                for key in gpi_results_avg:
                    if averaging == 'before':
                        ...
                        gpi_results_avg[key].append(np.mean(rotation_results[key][elm_time_ind-nwin:elm_time_ind+nwin],axis=0))                            
                        
                    elif averaging == 'elm':
                        gpi_results_avg[key].append(np.mean(rotation_results[key][elm_time_ind:elm_time_ind+n_elm], axis=0))
                    ...
                    if len(rotation_results[key][elm_time_ind-nwin:elm_time_ind+nwin].shape) == 2:
                        try:
                            ...
                            print(rotation_results[key][:,1].shape)
                            print(time)
                            print(elm_time_ind-nwin,elm_time_ind+nwin)
                        ...
                        try:
                            ...
                            print(key, rotation_results[key], elm_time_ind, nwin)
                            raise ValueError('asdf')
