...
try:
    ...
    h5py = None
    print('h5py not installed. HDF5 files won\'t be readable')
...
# The same camera data is also stored in per shot HDF5 files as
# <HDF5_DATAPATH>/<shot>.h5, holding a CAMERA_FRAMES group per camera.
HDF5_DATAPATH = os.getenv('NSTX_HDF5_DATAPATH',
                          '/p/nstxdigtwin/experimental_data/mdsplus/NSTX/data')
...
def hdf5_path(shot):
    """Full path of the HDF5 file belonging to a shot."""
    return os.path.join(HDF5_DATAPATH, str(shot) + '.h5')


class HDF5Cine:
    """PIMS-Cine lookalike reading the frames from an NSTX HDF5 file.

    Only the small part of the pims.Cine interface the player actually uses is
    implemented: len(), indexing, .shape, .frame_rate, .header_dict and
    .setup_fields_dict. The frames are read lazily, one at a time, so opening a
    file stays cheap no matter how many frames it holds.
    """

    # Datasets holding the video, in order of preference.
    FRAME_DATASET_NAMES = ('frames', 'data', 'video', 'images')

    def __init__(self, file_path, camera_alias=None, group_name=None):
        self._file = h5py.File(file_path, 'r')
        try:
            candidates = self._discover(self._file)
            if not candidates:
                raise ValueError(
                    'No camera frames found in ' + file_path +
                    '. Top level entries: ' +
                    (', '.join(list(self._file.keys())[:20]) or '<empty>'))

            if group_name is not None:
                selected = next((c for c in candidates
                                 if group_name in (c['path'], c['name'])), None)
                if selected is None:
                    raise ValueError('No group named ' + str(group_name) +
                                     ' in ' + file_path)
            else:
                selected = self._select(candidates, camera_alias)
        except Exception:
            self._file.close()
            raise

        self.group_name = selected['name']
        self.path = selected['path']
        self._frames = selected['dataset']
        self.attrs = selected['attrs']

        self.frame_rate = float(self.attrs.get('fps', 1.0) or 1.0)
        self.shape = tuple(self._frames.shape[1:])
        # The player expects the cine header fields; the HDF5 files always
        # start at the first recorded frame.
        self.header_dict = {'first_image_no': 0, 'compression': 0}
        self.setup_fields_dict = {'cfa': int(self.attrs.get('cfa', 0) or 0)}
        # Frames of a Bayer-encoded camera are already debayered to RGB in the
        # HDF5 file, so the player must not try to debayer them a second time.
        self.is_debayered = self._frames.ndim == 4 and self._frames.shape[-1] == 3
        self.real_bpp = int(self.attrs.get('real_bpp', 0) or 0)

    @classmethod
    def _discover(cls, h5_file):
        """Find every video dataset in the file, wherever it sits.

        The CAMERA_FRAMES group is not always at the root (some files nest it
        under the shot number or another wrapper group), so the whole file is
        walked instead of assuming a fixed path. A dataset counts as a video if
        it has 3 dimensions (grayscale) or 4 with 3 channels (RGB).
        """
        candidates = []
        ...
        def visit(name, obj):
            ...
            if base not in cls.FRAME_DATASET_NAMES:
                return
            shape = obj.shape
            if len(shape) == 3 or (len(shape) == 4 and shape[-1] in (1, 3, 4)):
                # The metadata lives on the parent group, but fall back to the
                # dataset's own attributes if the file stores them there.
                attrs = dict(obj.parent.attrs) if obj.parent is not None else {}
                if not attrs:
                    attrs = dict(obj.attrs)
                candidates.append({
                    'path': name,
                    'name': (obj.parent.name.rsplit('/', 1)[-1]
                             if obj.parent is not None and obj.parent.name != '/'
                             else base),
                    'dataset': obj,
                    'attrs': attrs,
                    })
    ...
    @staticmethod
    def _camera_folder(camera_alias):
        if camera_alias is None:
            return None
        return CAMERA_ALIASES.get(camera_alias, (None, None))[0]

    @classmethod
    def _select(cls, candidates, camera_alias):
        """Pick the entry belonging to the wanted camera.

        Every camera usually shows up twice, once for /p/nstxcam and once for
        the archive copy ("_archive" suffix). They hold the same video, so the
        non-archive one is preferred.
        """
        folder = cls._camera_folder(camera_alias)
        matching = candidates
        if folder is not None:
            matching = [c for c in candidates
                        if str(c['attrs'].get('camera_dir', '')) == folder
                        or folder in c['name']]
            if not matching:
                available = sorted(set(str(c['attrs'].get('camera_dir', '')) or c['name']
                                       for c in candidates))
                raise ValueError('No data of camera ' + str(camera_alias) +
                                 ' in the file. Available: ' +
                                 ', '.join(available))

        non_archive = [c for c in matching if not c['name'].endswith('_archive')]
        return sorted(non_archive or matching, key=lambda c: c['path'])[0]
    ...
    def __getitem__(self, index):
        ...
        frame = np.asarray(self._frames[index])
        # A stored (H, W, 1) frame is a grayscale image with a leftover channel
        # axis; PIL cannot build an image from it, so drop the singleton.
        if frame.ndim == 3 and frame.shape[-1] == 1:
            frame = frame[:, :, 0]
        # HDF5 keeps the byte order of the writing machine. PIL needs native
        # order, and a big endian uint16 would otherwise be scaled from
        # byte-swapped values.
        if frame.dtype.byteorder not in ('=', '|'):
...
class CinePlayer:
    ...
    def setup_gui(self):
        ...
        tk.Label(self.sidebar_frame, text="Source:", anchor="w").pack(side=tk.TOP, fill=tk.X, pady=(10, 0))
        self.source_var = tk.StringVar(value="Cine file")
        self.source_dropdown = ttk.Combobox(self.sidebar_frame,
                                            textvariable=self.source_var,
                                            values=["Cine file", "HDF5 file"],
                                            state="readonly", width=22)
        self.source_dropdown.pack(side=tk.TOP, fill=tk.X, pady=3)
    ...
    def open_file(self):
        ...
        if not file_path:
            return
        if self.load_file(file_path, camera_alias=self.camera_var.get()):
    ...
    def open_shot(self):
        ...
        camera_alias = self.camera_var.get()
        if self.source_var.get().startswith('HDF5'):
            file_path = hdf5_path(shot)
        else:
            file_path = cine_path(camera_alias, shot)

        if not os.path.exists(file_path):
            messagebox.showerror("Error", f"File not found:\n{file_path}")
            return
        if self.load_file(file_path, camera_alias=camera_alias):
    ...
    def load_file(self, file_path, camera_alias=None):
        try:
            ...
            if file_path.lower().endswith(('.h5', '.hdf5')):
                if not h5py_installed:
                    raise RuntimeError('h5py is not installed, cannot read HDF5 files.')
                self.reader = HDF5Cine(file_path, camera_alias=camera_alias)
            else:
        ...
        try:
            ...
            if cmap_name == "True Color (Debayer)":
                try:
                    ...
                    # Frames coming from an HDF5 file are stored as plain RGB
                    # arrays that were already debayered when the file was
                    # written. Debayering them again would destroy the colors.
                    if getattr(self.reader, 'is_debayered', False):
                        img = Image.fromarray(np.squeeze(norm_data), 'RGB')
                ...
                if flat_data.ndim == 2:
                    ...
                    # It already has colors; just display it. A colormap cannot
                    # be applied to an RGB frame, which is what the HDF5 files
                    # hold for the Bayer cameras.
                    img = Image.fromarray(flat_data)
            else:
                img = Image.fromarray(np.squeeze(norm_data))
...
def dump_hdf5_structure(file_path):
    """Print the tree of an HDF5 file, for checking what a shot file holds."""
...
if __name__ == "__main__":
    ...
    # "cine_player.py --dump-hdf5 <file>" prints the structure of a shot file
    # instead of starting the GUI.
    if len(sys.argv) > 2 and sys.argv[1] == '--dump-hdf5':
        dump_hdf5_structure(sys.argv[2])
        sys.exit(0)
