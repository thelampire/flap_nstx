...
def fit_nstx_profiles(exp_id=None,                                              #Shot number
        ...
        if nocalc and os.path.exists(pickle_filename):
            ...
            if test_t_diag:
                print('Fit 1st in ',time_mod.t_diag()-start_t_diag)
                start_t_diag=time_mod.t_diag()
            ...
            try:
                ...
                if ion: 
                    ...
                    data=data.T
                    error=error.T
                    data_validity=data_validity.T
                    
                t_diag=conn.get(f'dim_of({node},{t_dim})').data()              #For some reason the second coordinate is time, flap_mdsplus cannot read that.
            ...
            try:
                ...
                R_mag_axis_diag_time=np.interp(t_diag,
                                               R_mag_axis_efit_time,
                                               R_mag_axis_data)
                        
            except Exception as e:
                print(e)
                print('Failed to read Rmag_axis for shot ',exp_id)
                R_mag_axis_data=np.nan
                
            try:
                for ind_time, _ in enumerate(t_diag):
            ...
            psi_n_R_efit_t_diag=np.zeros([n_t_diag,n_R_efit])
            # print(radial_coordinate, R_on_grid, psi_N_on_grid)
            print(psi_n_efit.shape, R_efit.shape, t_efit.shape)
            
            for ind_r_efit in range(n_R_efit):
                psi_n_R_efit_t_diag[:,ind_r_efit]=np.interp(t_diag, 
                                                            t_efit, 
            ...
            #                                   psi_t_coord.shape[0]])
            psi_n_diag=np.zeros([t_diag.shape[0],data.shape[1]])
            
            for ind_t_diag in range(len(t_diag)):
                # ind_t_efit=np.argmin(np.abs(t_efit-t_diag[index_t]))
                psi_n_diag[ind_t_diag,:]=np.interp(R_diag,
                                                   R_efit,
                                                   psi_n_R_efit_t_diag[ind_t_diag,:])
                
                
            if test:
                for index_t in range(len(t_diag)):
                    plt.cla()
            ...
            if test_t_diag:
                print('Fit 2nd in ',time_mod.t_diag()-start_t_diag)
                start_t_diag=time_mod.t_diag()

                
            # print(t_diag.shape,d.data.shape, d.coordinate('Flux r')[0].shape)
            profile_data={'t_diag':t_diag,
                          'Data':data,
                          ...
                          'Fit parameters':np.zeros([t_diag.shape[0],5]),
                          'Fit parameter errors':np.zeros([t_diag.shape[0],5]),
                          'Fit parameter covariance':np.zeros([t_diag.shape[0],5,5]),
                         
                          'a':np.zeros(t_diag.shape),
                          'Height':np.zeros(t_diag.shape),
                          'Width':np.zeros(t_diag.shape),
                          'Global gradient':np.zeros(t_diag.shape),
                          'Position':np.zeros(t_diag.shape),
                          'Position r':np.zeros(t_diag.shape),
                          'SOL offset':np.zeros(t_diag.shape),
                          'Max gradient':np.zeros(t_diag.shape),
                          'Value at max':np.zeros(t_diag.shape),
                          'SOL avg':np.zeros(t_diag.shape),
                          }
            
            if modified_tanh:
                profile_data['Slope']=np.zeros(t_diag.shape)
            ...
            if modified_tanh:
                profile_data['Error']['Slope']=np.zeros(t_diag.shape)

            if test_t_diag:
                print('Fit 3rd in ',time_mod.t_diag()-start_t_diag)
                start_t_diag=time_mod.t_diag()

            for ind_t_diag, cur_time in enumerate(t_diag):
                
                
                data_validity_curr=data_validity[ind_t_diag,:]
                
                ind_valid_data=np.where(np.logical_and(data_validity_curr != 0.,
                                                       ~np.isnan(data[ind_t_diag,:])))
                
                data_curr=(data[ind_t_diag,:])[ind_valid_data]
                data_error_curr=error[ind_t_diag,:][ind_valid_data]
                
                psi_n_curr=psi_n_diag[ind_t_diag,:][ind_valid_data]
                R_diag_curr=R_diag[ind_valid_data]
                ...
                if r_coord_name =='Flux r':
                    ...
                    r_mag_axis_curr=R_mag_axis_diag_time[ind_t_diag]
                    ind_mag_axis=np.argmin(np.abs(R_diag_curr-r_mag_axis_curr))

                    x_data=x_data[ind_mag_axis:]
                    y_data=y_data[ind_mag_axis:]
                    y_data_error=y_data_error[ind_mag_axis:]

                else:
                    #By default it's outboard only, the full profile is not tanh
                    if average_profiles is not None and ind_t_diag+1 > average_profiles:
                        ind_max=np.argmax(data_curr)
                        x_data=R_diag_curr[ind_max:]
                        y_data=(np.sum(data_curr[ind_t_diag-average_profiles+1:ind_t_diag+1,ind_max:] *
                                       error[ind_t_diag-average_profiles+1:ind_t_diag+1,ind_max:]) /
                                average_profiles/np.sum(error[ind_t_diag-average_profiles+1:ind_t_diag+1,ind_max:],axis=1)
                                )
                        y_data_error=np.sqrt(np.sum(error[ind_t_diag-average_profiles+1:ind_t_diag+1,ind_max:]**2,axis=1))/np.sqrt(average_profiles)
                        
                    else:
                        ind_max=np.argmax(data_curr)
                        x_data=R_diag_curr[ind_max:]
                        y_data=data_curr[ind_max:]
                        y_data_error=error[ind_max:]
                ...
                try:
                    ...
                    print('Missing TS data for shot '+str(exp_id)+', t_diag: '+ str(t_diag[ind_t_diag]))
                ...
                if test or (plot_t_diag is not None and ind_t_diag==np.argmin(np.abs(plot_t_diag-t_diag))):
                    plt.cla()
                    ...
                    t_diag_string=' @ '+str(t_diag[ind_t_diag])
                    plt.title('Fit '+profile_string+' profile of '+str(exp_id)+t_diag_string)
                    plt.xlabel(xlabel)
                    plt.ylabel(ylabel)
                ...
                if modified_tanh:
                    profile_data['Fit parameters'][ind_t_diag,:]=popt
                    profile_data['Fit parameter errors'][ind_t_diag,:]=perr
                    profile_data['Fit parameter covariance'][ind_t_diag,:,:]=pcov
                    
                else:
                    profile_data['Fit parameters'][ind_t_diag,0:4]=popt
                    profile_data['Fit parameter errors'][ind_t_diag,0:4]=perr
                    profile_data['Fit parameter covariance'][ind_t_diag,0:4,0:4]=pcov

                profile_data['Height'][ind_t_diag]=popt[0]
                profile_data['SOL offset'][ind_t_diag]=popt[1]
                profile_data['Position'][ind_t_diag]=popt[2]

                try:
                #if True:
                    profile_data['Position r'][ind_t_diag]=np.interp(popt[2],
                                                                   psi_n_diag[ind_t_diag,np.argmin(psi_n_diag[ind_t_diag,:]):],
                                                                   R_diag[np.argmin(psi_n_diag[ind_t_diag,:]):])
                except:
                    print('Interpolation failed.')
                    profile_data['Position r'][ind_t_diag]=np.nan

                profile_data['Width'][ind_t_diag]=popt[3]
                
                if r_coord_name == 'Flux r':
                    ind_max=np.argmin(psi_n_diag[ind_t_diag,:])
                    ind_sol=np.where(psi_n_diag[ind_t_diag,ind_max:] > 1.0)
                    ...
                    ind_max=np.argmax(data[ind_t_diag,:])
                    ...
                    ind_sol=np.where(np.logical_and(R_diag[ind_max:] > R_separatrix, 
                                                    data[ind_t_diag,ind_max:] < sol_limit))
                profile_data['SOL avg'][ind_t_diag]=np.mean((data[ind_t_diag,ind_max:])[ind_sol])
                # profile_data['Error']['SOL avg'][ind_t_diag]=np.sqrt(np.var((d.data[ind_max:,ind_t_diag])[ind_sol]))
                profile_data['Error']['SOL avg'][ind_t_diag]=np.sqrt(np.sum(error[ind_t_diag,ind_max:][ind_sol]**2))/len(ind_sol)
                
                if modified_tanh:
                    profile_data['Slope'][ind_t_diag]=popt[4]

                profile_data['Error']['Height'][ind_t_diag]=perr[0]
                profile_data['Error']['SOL offset'][ind_t_diag]=perr[1]
                profile_data['Error']['Position'][ind_t_diag]=perr[2]
                profile_data['Error']['Width'][ind_t_diag]=perr[3]
                profile_data['Error']['Value at max'][ind_t_diag]=(perr[0]+perr[1])/2
                profile_data['Error']['Global gradient'][ind_t_diag]=(perr[0]/popt[3]+
                                                                          perr[1]/popt[3]+
                                                                          np.abs((-popt[1]+popt[0])/popt[3]**2)*perr[3])
                profile_data['Error']['Max gradient'][ind_t_diag]=(np.abs(1/(4*popt[3])*perr[1])+
                                                                       np.abs(1/(4*popt[3])*perr[0]))

                if modified_tanh:
                    profile_data['Error']['Slope'][ind_t_diag]=perr[4]
            ...
            if test_t_diag:
                print('Fit 4th in ',time_mod.t_diag()-start_t_diag)
